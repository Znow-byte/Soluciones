...
function Machinery() {
    ...
    // recalcula cada vez que la pantalla cambie de tamaño
    useEffect(() => {
        const updateMaxOffset = () => {
        if (wrapperRef.current) {
            const wrapperWidth = wrapperRef.current.clientWidth;
            const scrollWidth = wrapperRef.current.scrollWidth;
            setMaxOffset(Math.max(scrollWidth - wrapperWidth, 0));
            // aseguramos que el offset nunca supere el maxOffset
            setOffset((prev) => Math.min(prev, Math.max(scrollWidth - wrapperWidth, 0)));
        }
        };
    ...
    const scroll = (direction) => {
        ...
        setOffset((prev) => {
            if (direction === "next") {
                ...
                return Math.max(prev - cardWidth, 0);
            }
        });
    };
    ...
    return (
        ...
        <div className="machinery-row">
            ...
            <div className="machinery-wrapper" ref={wrapperRef}>
                <div
                    className="machinery-cards"
                    style={{ transform: `translateX(calc(-${offset}px))` }}
                >
